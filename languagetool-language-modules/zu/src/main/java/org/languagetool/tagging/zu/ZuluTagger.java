 ...
 * USA
 */
package org.languagetool.tagging.zu;
...
/**
 * A part-of-speech tagger for Zulu.
 *
 * @author Dominique Pellé
 */
public class ZuluTagger implements Tagger {
  // manual tagger is used to tag the list of closed Zulu words
  ...
  private synchronized void lazyInit() throws IOException {
    ...
    // limited set of words in Zulu which have no standard ending, as
    // opposed to open words which are unlimited in numbers and which follow
    // strict rules for their suffixes.
    try (InputStream stream = JLanguageTool.getDataBroker().getFromResourceDirAsStream("/zu/manual-tagger.txt")) {
    ...
    setTransitiveVerbs   = loadWords(JLanguageTool.getDataBroker().getFromRulesDirAsStream("/zu/verb-tr.txt"));
    setIntransitiveVerbs = loadWords(JLanguageTool.getDataBroker().getFromRulesDirAsStream("/zu/verb-ntr.txt"));
    setNonParticiple     = loadWords(JLanguageTool.getDataBroker().getFromRulesDirAsStream("/zu/root-ant-at.txt"));
  }
  ...
  public List<AnalyzedTokenReadings> tag(List<String> sentenceTokens) throws IOException {
    ...
    for (String word : sentenceTokens) {
      ...
      if (word.length() > 50) {
        ...
        // No Zulu word is made of one letter only. This check avoids
